...
$(function() {
  if ('undefined' !== typeof flipbookJSON && "Invalid JSON" !== flipbookJSON && 'undefined' !== typeof flipbookJSON.pages && flipbookJSON.pages.length > 0) {
    br.dataJSON = flipbookJSON;
    br.initializeBookReader();
    br.init();
  } else {
	$('.initMessage').addClass('initMessageError').text('This book has no valid pages to display');
  }
...
br.initializeBookReader = function() {
  ...
  br.getPageURI = function(index, reduce, rotate) {
    ...
    return this.updateProtocol(this.dataJSON.pages[index]['stacksURL']) + '_' + bestSize;
  }


  br.getThumbImgURI = function(index) {
    var lowestSize = "thumb";
    return this.updateProtocol(this.dataJSON.pages[index]['stacksURL']) + '_' + lowestSize;
  }

  br.updateProtocol = function(stacksURL) {
    if (/^Stanford$/i.test(this.dataJSON.readGroup)) {
      stacksURL = stacksURL.replace(/http:/, 'https:');
    }
  ...
  br.getJp2LevelLargerThanContainer = function(ctWidth, ctHeight, index) {
    ...
    ctLevel = this.clampJP2Level(index, ctLevel + 2); // two levels larger than best fit
    return ctLevel;
  };
